...
export default function AdminScreen() {
    ...
    return `
      ...
      <body>
        ...
        <div class="evidence-section">
          <div class="evidence-title">⚡ ECG Recordings - Direct Print Access</div>
          <div class="police-notice">
            📋 SUPER ADMIN DIRECT ACCESS 📋<br/>
            Actual ECG recordings from patient care - Ready for immediate printing<br/>
            All ECG captures embedded and printable without restrictions
          </div>
          ...
          ${relatedECGs.length > 0 ? relatedECGs.map((ecg, index) => `
            <div class="ecg-section">
              ...
              <div style="font-size: 8pt; color: #999; margin-top: 5px;">
                ECG ID: ${ecg.ecg_id} | Direct Access: Ready for printing
              </div>
            </div>
          `).join('') : ''}
          ...
          ${pcr.vitals.filter(v => v.ecgCapture).map((vital, index) => `
            <div class="ecg-section">
              ...
              `}
              <div style="font-size: 8pt; color: #999; margin-top: 5px;">
                ECG ID: ECG_VIT_${pcr.id}_${index} | Captured: ${vital.timestamp}
              </div>
            </div>
          `).join('')}
        </div>
        ...
        <div class="evidence-section">
          <div class="evidence-title">✍️ Patient Care Signatures - Direct Print Access</div>
          <div class="police-notice">
            📋 SUPER ADMIN DIRECT ACCESS 📋<br/>
            Actual signatures from patient care documentation - Ready for immediate printing<br/>
            All signatures embedded and printable without restrictions
          </div>
        ...
        <div class="evidence-section">
          <div class="evidence-title">📋 Direct Medical Evidence Access</div>
          <div style="font-size: 10pt; line-height: 1.4;">
            <div><strong>Report Created:</strong> ${pcr.submittedAt}</div>
            <div><strong>Generated:</strong> ${new Date().toISOString()}</div>
            <div><strong>Evidence Type:</strong> Direct access to actual signatures and ECG captures</div>
            <div><strong>Super Admin Access:</strong> Complete printable medical evidence without restrictions</div>
            <div><strong>Signature Access:</strong> All electronic signatures embedded and printable</div>
            <div><strong>ECG Access:</strong> All ECG recordings embedded at full resolution</div>
            <div style="margin-top: 10px; padding: 10px; background-color: #e8f5e8; border: 1px solid #4caf50;">
              <strong>✅ DIRECT ACCESS ENABLED:</strong> This document provides direct access to actual medical evidence. 
              All signatures and ECG captures are embedded in this document and will appear in printed copies. 
              Super Admin has unrestricted access to all patient care documentation for printing and review purposes.
            </div>
          </div>
        </div>
        ...
        <div class="footer">
          ...
          <div style="margin-top: 10px; font-size: 8pt; color: #666;">
            This document contains complete patient care information with all signatures and ECG captures embedded.<br/>
            All medical evidence is directly accessible and will print clearly on paper.
          </div>
          <div style="margin-top: 10px; padding: 8px; background-color: #e8f5e8; border: 1px solid #4caf50; font-size: 8pt;">
            <strong>📋 SUPER ADMIN PRINTABLE DOCUMENT 📋</strong><br/>
            This report contains actual signatures and ECG recordings embedded for direct printing.<br/>
            All medical evidence is immediately accessible without digital verification requirements.<br/>
            <strong>COMPLETE ACCESS:</strong> Super Admin can print all patient signatures and ECG captures directly.<br/>
            <strong>PRINT READY:</strong> All signatures and ECG images will appear clearly in printed documents.<br/>
            <strong>DIRECT ACCESS:</strong> No digital chain requirements - immediate access to all medical evidence.
          </div>
        </div>
      </body>
      </html>
  ...
  const handleExportPDF = async (isComprehensive: boolean = false) => {
    ...
    if (isComprehensive && !isSuperAdmin) {
      Alert.alert(
        ...
        'Only Super Admin can download complete patient care reports with actual signatures and ECG captures for printing. This includes:\n\n• All actual signatures (Nurse, Doctor, Patient/Guardian)\n• All ECG recordings and captures\n• Complete printable medical documentation\n• Direct access to all patient care evidence',
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    try {
      ...
      const fileName = isComprehensive 
        ? `SUPERADMIN_DIRECT_ACCESS_PCR_${selectedPCR.id}_${new Date().toISOString().split('T')[0]}.pdf`
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        Alert.alert(
          ...
          isComprehensive 
            ? `📋 SUPER ADMIN PRINTABLE REPORT READY 📋\n\nGenerated: ${fileName}\n\nThis PDF contains:\n• All actual signatures embedded and printable\n• All ECG captures at full resolution\n• Complete patient care documentation\n• Direct access medical evidence\n\nReady for immediate printing with all signatures and ECG clearly visible.`
            : 'Professional PDF report generated successfully'
        );
      }
      
      await addAuditLog(
        isComprehensive ? 'EXPORT_SUPERADMIN_DIRECT_ACCESS_PDF' : 'EXPORT_PDF', 
        'PCR', 
        selectedPCR.id, 
        isComprehensive 
          ? `Super Admin exported direct access PDF with all actual signatures and ECG captures for immediate printing`
  ...
  const handleExportAllData = async () => {
    if (!isSuperAdmin) {
      Alert.alert(
        'Super Admin Access Required',
        'Only Super Admin can export complete system data with all actual signatures and ECG captures for printing.',
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Super Admin Complete Export Ready',
        'All system data with actual signatures and ECG captures has been copied to clipboard. This includes all PCRs, patient data, vitals, ECGs, signatures, transport info, staff records, and audit logs with direct access.',
        [{ text: 'OK' }]
      );
      await addAuditLog('SUPERADMIN_EXPORT_ALL_DATA', 'System', 'ALL', 'Super Admin exported complete system data with actual signatures and ECG captures');
    } catch (error) {
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.dataCardActions}>
                    ...
                    {isSuperAdmin && (
                      <TouchableOpacity
                        style={[styles.actionButton, { borderColor: '#d32f2f', backgroundColor: '#ffebee' }]}
                        onPress={() => {
                          setSelectedPCR(pcr);
                      ...
                      >
                        ...
                        <Text style={[styles.actionButtonText, { color: '#d32f2f', fontWeight: 'bold' }]}>📋 Print Direct</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                </TouchableOpacity>
            ...
            <View style={styles.bulkActionButtons}>
              ...
              {isSuperAdmin && (
                <TouchableOpacity style={[styles.bulkActionButton, { backgroundColor: '#ff6b35' }]} onPress={() => {
                  Alert.alert(
                    '📋 Super Admin Medical Evidence Access',
                    'DIRECT ACCESS TO MEDICAL EVIDENCE:\n\n📋 SIGNATURES AVAILABLE:\n• Actual nurse signatures ready for printing\n• Doctor signatures embedded and printable\n• Patient/Guardian consent signatures\n• Direct signature access without verification\n\n⚡ ECG CAPTURES AVAILABLE:\n• All ECG recordings from patient care\n• High-resolution ECG strips for printing\n• Complete rhythm analysis and notes\n• Multiple ECG captures per patient case\n\n🖨️ PRINTING CAPABILITIES:\n• All signatures print clearly on paper\n• All ECG images at full printable resolution\n• Professional medical documentation format\n• Complete patient care evidence\n\n📋 SUPER ADMIN ACCESS:\nSuper Admin has direct access to print all patient signatures and ECG captures immediately without digital verification requirements.',
                    [{ text: 'Understood' }]
                  );
                }}>
    ...
    <View style={styles.container}>
      {isSuperAdmin && (
        <View style={styles.superAdminBanner}>
          ...
          <Text style={styles.superAdminText}>📋 SUPER ADMIN - DIRECT ACCESS ENABLED</Text>
          <Text style={styles.superAdminSubtext}>Direct access to all patient reports, signatures & ECG captures for immediate printing</Text>
        </View>
      )}
            ...
            <View style={styles.reportActions}>
              ...
              {isSuperAdmin && (
                <TouchableOpacity style={[styles.reportActionButton, { borderColor: '#d32f2f', backgroundColor: '#ffebee' }]} onPress={() => handleExportPDF(true)}>
                  ...
                  <Text style={[styles.reportActionText, { color: '#d32f2f', fontWeight: 'bold' }]}>📋 Super Admin Direct Access PDF</Text>
                </TouchableOpacity>
              )}
              ...
              {isSuperAdmin && (
                <TouchableOpacity style={[styles.reportActionButton, { borderColor: '#ff6b35', backgroundColor: '#fff3e0' }]} onPress={() => {
                  Alert.alert(
                    ...
                    'DIRECT ACCESS SIGNATURES & ECG PRINTING:\n\n✅ SIGNATURES:\n• All actual signatures embedded in PDF\n• Nurse, Doctor, and Patient/Guardian signatures visible\n• Direct signature access without verification\n• Clear timestamps and identification\n\n✅ ECG CAPTURES:\n• All ECG recordings embedded as high-quality images\n• Full-resolution ECG strips ready for printing\n• Complete rhythm data and timestamps\n• Multiple ECG captures per patient supported\n\n✅ PRINT READY:\n• PDF optimized for standard paper sizes\n• All images print clearly and legibly\n• Professional medical documentation format\n• Direct access without digital verification\n\n📋 DIRECT ACCESS:\nSuper Admin can immediately print all medical evidence with signatures and ECG captures clearly visible.',
                    [{ text: 'Understood' }]
                  );
                }}>
