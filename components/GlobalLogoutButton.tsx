...
interface GlobalLogoutButtonProps {
  variant?: 'button' | 'tab' | 'header' | 'floating' | 'minimal' | 'emergency';
  showText?: boolean;
  iconSize?: number;
  color?: string;
...
/**
 * Global logout button that works from any screen or form
...
export const GlobalLogoutButton: React.FC<GlobalLogoutButtonProps> = ({
  ...
  const { logout, emergencyLogout, isLoggingOut, isLoggedIn, currentSession } = useGlobalLogout();
  ...
  const handleLogout = () => {
    if (variant === 'emergency') {
      emergencyLogout('Emergency logout button pressed');
    } else {
      logout({
        showConfirmation,
        confirmTitle,
    ...
    return (
      <View style={styles.floatingContainer}>
        <TouchableOpacity
          ...
          testID={testID || `global-logout-${variant}`}
